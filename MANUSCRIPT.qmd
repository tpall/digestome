...
No existing tool closes the gap, and the distinction is finer than KEGG dependence alone.
DRAM can already be run without KEGG, but its remaining annotation sources for carbohydrate
and peptidase function are dbCAN and MEROPS, which are themselves restricted, so a KEGG-free
configuration is still not a commercially clean one. That is the gap: not *KEGG-free*, but
*free of every licence-encumbered dependency in the stack*.
...
METABOLIC queries KOfam and interprets hits in the context of KEGG Modules; KEMET evaluates
KEGG Module completeness and ships the KEGG licence with its source; MetaPathPredict predicts
completeness of KEGG Modules directly. DRAM is the only one that can be run KEGG-free, and
that configuration still depends on dbCAN and MEROPS. The dependency is not incidental to any
of them: KEGG Modules *are* the pathway model, so removing KEGG means having no pathway
definitions left, which is why a replacement has to supply its own curated model rather than
simply swapping a database.
