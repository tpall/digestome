...
No existing tool closes the gap, and the reason is structural rather than accidental: each
available option satisfies three of the four requirements and misses a different one.

| | workflow-managed | MAG-level | module-level | licence-clean |
|---|---|---|---|---|
| DRAM [@shaffer2020dram] | ✗ | ✓ | ✓ | ✗ (dbCAN, MEROPS) |
| METABOLIC [@zhou2022metabolic] | ✗ | ✓ | ✓ | ✗ (KEGG KOfam) |
| nf-core/mag [@krakau2022mag] | ✓ | ✓ | ✗ | ✗ (KEGG) |
| nf-core/metatdenovo [@metatdenovo2025] | ✓ | ✗ | ✗ | ✗ (eggNOG, KofamScan) |
| this work | ✓ | ✓ | ✓ | ✓ |

METABOLIC and DRAM reconstruct metabolism at module level but resolve it through KEGG, and
DRAM additionally depends on dbCAN and MEROPS for carbohydrate and peptidase annotation. The
Nextflow pipelines that exist stop earlier: nf-core/mag ends at binning and taxonomic
classification, and nf-core/metatdenovo produces per-gene annotated catalogues without binning,
explicitly directing users elsewhere for MAGs. Both route functional annotation through KEGG or
eggNOG. Routing through eggNOG-mapper does not evade the constraint, since the KEGG orthology
identifiers it emits are themselves KEGG content.
