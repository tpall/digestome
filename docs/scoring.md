...
Lineage confirms only at genus level, from the policy table `assets/acetate_lineages.tsv` (role `acetoclastic`): *Methanosarcina*, *Methanothrix* (*Methanosaeta*) and *Methanocrinis* (formerly *Methanothrix harundinacea*). GTDB suffixes are ignored (`Methanothrix_B`). A family does not confirm: Methanosarcinaceae is mostly methylotrophs that do not use acetate (*Methanolobus*, *Methanococcoides*, *Methanohalophilus*, *Methanomethylovorans*, *Methanimicrococcus*), and an unnamed genus, even in Methanotrichaceae, is scored absent on taxonomy and left for review. The same table lists (role `anabolic_acetate_genes`) the methanogen orders whose members carry the acetate-route genes for biomass; report renderers read it to tell an expected gene hit from a candidate. `tests/test_acetoclastic_lineage.py` checks the rule and, with `GTDB_TAXONOMY` set, that every name in the table exists verbatim in the GTDB taxonomy in use.
