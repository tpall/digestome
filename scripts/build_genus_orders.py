...
that maps to more than one order is left out (ambiguous).
...
for line in open(sys.argv[1]):
    ...
    for name in (ranks.get('g', ''), f):
        if name:
            orders[strip(name)].add(strip(o)); families[strip(name)].add(strip(f))
    if o:
        orders[strip(o)].add(strip(o))
...
for name in sorted(orders):
    if len(orders[name]) == 1:
        ...
        print(f'{name}\t{o}\t{fam if name != o else ""}')
